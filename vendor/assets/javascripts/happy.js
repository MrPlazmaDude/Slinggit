  ...
  $.fn.isHappy = function (config) {
    ...
    function processField(opts, selector) {
      var field = $(selector),
        ...
        errorEl = $(errorCode.id).length > 0 ? $(errorCode.id) : getError(errorCode);
      ...
      field.testValid = function (submit) {
        ...
        if (error) {
          // CMK: This whole if block was added by me to take
          // methods as the message.  However, it proved to be pretty
          // clunky, so leaving it for now.
          if( isFunction(opts.message) ){
            errorCode.message = opts.message()
            if($("#"+errorCode.id).length > 0){
              $("#"+errorCode.id).empty().text(errorCode.message);
            } else {
              errorEl = getError(errorCode)
            }  
          }
